**中文导读：** 查找 behavior-equivalent 的 existing utilities、standard-library/runtime primitives，以及 platform/framework/downstream 已保证却被手工重复维护的 behavior。任何替换都必须保持 outputs、errors、side effects 和 ordering；无法证明等价时跳过。下方英文是 reviewer 的 canonical prompt contract，保持原样执行。
