...
**中文导读：** 对当前 branch diff 或明确指定的 code surface 做 behavior-preserving refinement，优先提升 clarity、reuse 和 maintainability，并通过现有 tests 与 relevant checks 证明没有改变行为。User-named scope 是权威 mutation boundary，只能额外修改其中必要的 import/export seams；reviewer dispatch 的非 backpressure failure 要以相同 prompt inline 补做。只有确认 current unshipped scope 的旧 interface/data shape 没有 deployed、persisted、public、external、dependent-branch 或 scope 外 caller 时，才可移除 compatibility path。不要越过 plan 中的 structure pins，也不要把 unrelated cleanup 混入本次 simplification。下方英文内容是 canonical executable contract，必须按原文执行。
