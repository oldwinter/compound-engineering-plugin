...
**中文导读：** 基于当前 project 形成 decisive POV，输出形状随 subject 变化：adoption 使用 graded verdict，document 使用 holistic take，既有 approaches 使用 project-grounded position。可按用户要求由 named peers 或 `oracle` 做 different-model cross-check；ce-pov 自己仍是 decision-maker，peer failure 不会 block solo POV。任何 project context 外发前都必须说明接收方与 route。下方英文内容是 canonical executable contract，必须按原文执行。
