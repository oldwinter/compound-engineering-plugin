...
Attribute an account authentication failure only after provider-capable dispatch is positively established. Without that proof, authentication-shaped peer text describes only the peer's execution context; never report it as the user's account being logged out, and provide login or credential-refresh remediation only when the provider-capable boundary is proven.
