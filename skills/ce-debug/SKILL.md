...
**中文导读：** 面向真实 bug、failed test、regression 或错误行为，先复现并沿完整 causal chain 找到 root cause，再决定是否实施 test-first fix。不要把症状猜测当原因，也不要在没有 evidence 的情况下连续试补丁；验证必须覆盖用户实际遇到的 surface。下方英文内容是 canonical executable contract，必须按原文执行。
...
**中文说明：** `mode:pipeline` 完全 non-interactive；先读取 `references/pipeline-mode.md`。只 fix convergent bugs，divergent/product-decision items 返回 `needs-human`，绝不调用 blocking-question tool。
...
**Residuals**: [none / accepted Known Residuals for PR / filed as tracker tickets / accepted residuals written to docs/residual-review-findings/<branch-or-head-sha>.md (last resort) / blocked pending user decision]
