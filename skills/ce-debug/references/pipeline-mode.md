...
**中文导读：** 本 reference 是 `ce-debug mode:pipeline` 的 canonical executable contract。它继承 orchestrator 已获授权的 scope，只允许在 current branch 上 fix/commit/push；禁止 merge、rebase、force-push 和 approve gated CI。只应用 convergent fixes，divergent 或需要 product/design decision 的事项以 structured `needs-human` residual 返回。
