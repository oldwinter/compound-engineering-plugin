...
**中文导读：** 对 PR 或 branch 影响到的 pages 运行 E2E browser tests。优先使用 active harness 提供的 capable host-native integrated browser；否则 fallback 到 `agent-browser`。Driver 一旦开始测试就不能混用。禁止另行安装或替换为 standalone Playwright、Puppeteer、separately configured browser extension/MCP 或 ad hoc automation。下方英文内容是 canonical executable contract。
