...
**中文导读：** 仅当 main skill 判定不存在 qualifying host-native integrated browser，并选择 `agent-browser` fallback 后读取本文件。先验证 CLI 和 browser runtime，再按下方 canonical commands 执行；不得把 runtime launch error 误报为 CLI 缺失。
