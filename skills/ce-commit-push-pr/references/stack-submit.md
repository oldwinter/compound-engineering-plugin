...
**中文导读：** 本 reference 只在 commit-push-pr 的 stack mode 已由 user intent 或 standing preference 选择后加载。它把生命周期分成 submit 前的 Probe/Topology/Retrospective construction 与 Step 5 独占的 Submit；必须保留已有 managed topology，只有 review boundaries 清晰时才按 whole-file groups 或 existing commit boundaries 构建最小 linear layers。Draft、published history rewrite、archive 与缺失 `gh stack` 的处理都按下方 residual/confirmation 规则执行。下方英文是 canonical executable contract，命令和安全边界保持原样。
