...
如果上方 line 解析为 plain branch name（如 `feat/my-branch`），在 Phase 1 session-history filtering 中使用它，这样 orchestrator 不会浪费一轮去推导。如果它仍包含 backtick command string 或为空，在 runtime 推导 branch。

**Repo root（pre-resolved）：** !`git rev-parse --show-toplevel 2>/dev/null || pwd`

如果上方 line 解析为 absolute path，在 Phase 1 中将它用作 session-history repo filter。如果它仍包含 backtick command string 或为空，在 runtime 用 `git rev-parse --show-toplevel 2>/dev/null || pwd` 推导 repo root。
...
- `references/agents/session-historian.md`：optional session-history compounding context 的 skill-local synthesis prompt（仅当用户 opt into session history 时读取）
- `assets/resolution-template.md`：new docs 的 section structure（assembling 时读取）
- `scripts/session-history/`：复制到此 skill 内的 session discovery 和 extraction scripts，因此 session-history support 不依赖已删除的 public `ce-sessions` skill
...
#### 4. **Session History**（启动 parallel block 后的 internal flow，仅当用户 opt in）
   - 如果用户在 follow-up question 中拒绝 session history、正在 lightweight mode 运行，或正在 headless mode 运行，则 **完全跳过**。
   - 直接在此 skill 中使用 `scripts/session-history/` 运行 session discovery、branch/keyword filtering、scan-window selection、deep-dive selection 和 per-session extraction。
   - 读取 skill-local synthesis prompt `references/agents/session-historian.md`，然后用该 prompt content dispatch generic subagent。Do not dispatch a standalone agent by type/name。

   **Session-history payload：保持 tight。** 冗长、keyword-rich 的 payload 会让 session-history flow 继续扩大范围。使用此形状：
   ...
   不要追加 additional context blocks、exclusion lists 或 topic-keyword bullets：verbose payloads 会授权 session-history flow 继续扩大 search，并快速 compound wall time。如果需要 keyword search，internal flow 会基于 topic 自行决定。
   - 返回：prior sessions 的 findings structured digest，或未找到时返回 "no relevant prior sessions"。
   - **Session history 是最终 Phase 1 input，不是 workflow stop。** 它返回后，将其 output 作为最后一个 input，直接进入 Phase 2：不要 emit summary，也不要暂停等待用户。"no relevant prior sessions" 返回仍是 valid input；documentation 会在没有 session context 的情况下写入。

   **Script resolution。** Claude Code 中，通过 `${CLAUDE_SKILL_DIR}/scripts/session-history/` 运行 bundled scripts。在 `${CLAUDE_SKILL_DIR}` 不可用且无法从 loaded skill directory resolve script path 的平台，visibly skip session history："Session history was requested, but this platform did not expose the bundled session-history scripts to the runtime." 继续 Phase 2，不带 session context。

   **Discovery pipeline。** 从 problem topic 推断 scan window，默认从 7 days 开始。运行 discovery 和 metadata extraction：
   ...
   如果 `_meta.files_processed` 为 `0`，返回 `no relevant prior sessions`。如果 first pass 没有 relevant branch matches，从 topic 派生 2-4 个 keywords，并用 `--keyword K1,K2,...` 重新运行 metadata extraction。最多保留 5 个 sessions（Claude Code、Codex 和 Cursor），按 branch match、keyword match count、file size over 30KB 和 recency 排名。排除 current session。

   **Extraction pipeline。** 创建 `SCRATCH=$(mktemp -d -t ce-compound-sessions-XXXXXX)`。对每个 selected session，将 extracted content 写入 scratch files：
   ...
   当 dead ends 或 recurring errors 可能有用时，selectively 使用 `extract-errors.py`。只将 scratch file paths 和 metadata 传给 synthesis subagent。

   **Synthesis dispatch。** 构建 generic subagent prompt，包含：
   - `references/agents/session-historian.md` 的完整内容
   - `problem_topic`
   - `scratch_dir`
   - 带 extracted file paths 和 metadata 的 `sessions` array
   - 上方 output schema
   - 上方 filter rule

   Subagent 只读取 scratch paths，并返回 prose findings。如果 synthesis 失败，note failure，并在没有 session context 的情况下继续。

### Phase 2：Assembly & Write（组装与写入）

<sequential_tasks>

**继续前等待所有 Phase 1 inputs 完成**：三个 parallel subagents，以及用户 opt in 时的 internal session-history flow。Session history 即使运行在 orchestrator 内，也是 Phase 1 input。
...
3. **纳入 session history findings**（如果可用）。当 internal session-history flow 返回 relevant prior-session context 时：
...
根据 problem type，可选读取 skill-local prompt assets，并 dispatch generic subagents review documentation。不要按 type/name dispatch standalone agents：

- **performance_issue（性能问题）** → `references/agents/performance-oracle.md`
- **security_issue（安全问题）** → `references/agents/security-sentinel.md`
- **database_issue（数据库问题）** → `references/agents/data-integrity-guardian.md`
- 任何 code-heavy issue → 对 drafted examples 和 explanatory claims 执行 read-only documentation review，确保 examples minimal 且清晰。Do **not** invoke `ce-simplify-code`；do not mutate product code。Diff 中的 structural concerns 在同一 work 经过 `/ce-code-review`（maintainability persona）时已经覆盖。
...
## Applicable Specialized Prompt Assets（适用的专家 Prompt 资产）

基于 problem type，这些 skill-local prompt assets 可增强 documentation。读取对应 `references/agents/*.md` 文件后，dispatch generic subagent；不要依赖 platform-level custom-agent registration。

### Code Quality & Review（代码质量与 Review）
- **code-simplicity-reviewer**：确保 solution code minimal 且清晰
- **pattern-recognition-specialist**：识别 anti-patterns 或 repeating issues

### Specific Domain Experts（特定领域专家）
- **performance-oracle**：分析 performance_issue category solutions
- **security-sentinel**：review security_issue solutions 中的 vulnerabilities
- **data-integrity-guardian**：review database_issue migrations 和 queries

### Enhancement & Research（增强与研究）
- **best-practices-researcher**：用 industry best practices 丰富 solution
- **framework-docs-researcher**：链接 framework/library documentation references

### When to Invoke（何时调用）
- **Auto-triggered（自动触发）** (optional): Prompt assets can run post-documentation for enhancement
- **Manual trigger（手动触发）**: User can ask for deeper review after /ce-compound completes
