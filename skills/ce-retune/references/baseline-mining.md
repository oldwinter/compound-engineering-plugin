...
本阶段只读取已有归档，不运行 harness，也不产生模型调用。先从语料库主流程建立“阶段 -> 可观察副作用”的 marker map；marker 必须是 tool call 或实际存在的文件，不能采用 assistant 自述。随后用脚本为每次运行抽取一行数据，至少保留运行与模型标识、父子 session、阶段轨迹、终止标记、token、时长、helper dispatch 和完整最终消息。

分析时把 `broken` 作为独立结果并检查其是否在各组均匀分布；把“遵循流程”和“完成任务”分成两列；使用 `output_tokens / wall_clock_min` 识别等待或停滞；按模型、设置、时间和并发等因素分层，避免把混杂因素当作模型效果。所有结论都必须能回溯到原始 trace。下方英文是精确分析契约，字段、枚举、公式和判定条件保持不变。
