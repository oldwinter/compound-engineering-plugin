...
一次 pass 只处理一个问题类别。先写 ownership manifest，把 corpus 按互不重叠的文件集合分给各执行单元；共享且必须字节一致的资产只能有一个 owner。涉及跨文件固定字符串时，先串行写出 canonical contract，再按文件所有权并行应用。任何 agent 都必须同时收到允许修改和禁止修改的精确路径。

所有修改完成后逐块 reconcile，再统一运行项目测试。测试锁定了已删除字符串时，应把测试路径作为 finding 报告，不能修改测试来迎合裁剪。随后收集 applied/skipped 结果、重新测量，并把该 pass 单独提交。下方英文是精确执行契约，manifest 字段、隔离条件、reconcile 步骤和停止规则保持不变。
