...
本阶段在任何修改之前，用同一 commit 的两个字节一致副本执行 A/A，测出“什么都不改”时的波动。两组必须使用同一 runner、任务与模型配置，运行顺序按 A/B、B/A 交错；先用一次运行证明 build selector 确实生效，并从运行完成后的持久字段读取 provenance。每行数据缺少 build 标签都应直接失败，不能按顺序或时间推断分组。

小样本使用 Wilson interval、risk difference confidence interval 与 Fisher exact test，同时报告 token spread；不能用 Wald 或正态近似。结果不显著时打印达到 80% power 所需的每组样本数。若已独立建立基线，可采用连续 N 次 clean run 的单臂 streak，但首个失败即停止，诊断运行不能计入 streak，任何编辑都会把计数重置为零。修改前把 metric、effect size、设计、显著性、停止规则和未覆盖阶段写入可持久读取的注册文件。下方英文是精确测量契约，公式、字段与统计判定保持不变。
