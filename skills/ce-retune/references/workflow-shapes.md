...
为每个阶段选择与证据形态匹配的编排方式：归档挖掘保持本地串行且可重复；A/A 与 A/B 在组内串行、组间交错，测量期间不运行其他会造成争用的工作；corpus audit 按 unit 分派并把 proposer 结果流水线式交给 defender；跨单元改写先由单一 owner 写 contract，再并行应用；cut pass 按互不重叠的文件所有权分派，共享资产只交给一个 owner，并在验证前设置 barrier；验证按独立检查维度分派；最后由一个没有参与修改的 critic 检查未落地项。

所有可聚合结果都要先定义 schema，所有阶段产物都要在执行前确定持久路径。并发上限以上的 dispatch 会排队，应显式计算波次；列出未运行、跳过和未被任何 owner 覆盖的单元。下方英文是精确编排契约，阶段顺序、隔离规则、artifact 要求和 coverage 声明保持不变。
