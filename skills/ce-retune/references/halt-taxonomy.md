...
本参考用于定位工作流无故停在中途的文本模式。首先必须判断阶段边界是真实 runtime 边界，还是同一 conversation 中通过 skill load、include 或文件读取形成的虚构边界：只有 trace 中存在独立 dispatch tool call、子 session id 且结果返回父 trace，才算真实 hand-off。缺少 session id 时采用这一结构性证据，不能把普通读取当成独立执行者。

逐类 grep、人工阅读命中项并做可证伪检查，禁止批量替换。修复重点包括：等待并不存在的并行 helper、向不存在的 caller hand-off、步骤末尾没有继续指令、只有完成定义却无后继动作等；同时保留安全确认、授权边界和产品本来就要求暂停询问的 stop。字段、枚举、greppable marker 和 security guard 都是数据，不得改动。下方英文是精确分类与替换契约。
