...
> **中文导读：** Destination 通过当前 session capability 探测决定，不能因为 binary/env/MCP tool 尚未加载就断言不可用。Local file 永远是 fallback；其它 destination 只负责发布 canonical output，不得改写内容 contract。
