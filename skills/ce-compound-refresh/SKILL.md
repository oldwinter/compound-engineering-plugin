...
> **中文导读（下方英文为 canonical contract）：** `ce-compound-refresh` 会基于当前 codebase 证据审阅 `<root>/solutions/`，对 stale、drift、重叠或已被替代的 learning 执行可证明的维护动作，并输出逐文件 report。默认 interactive；使用 `mode:non-interactive` 时不提问，安全动作直接执行，无法判定的内容标记为 stale，迁移与拆分遵守更严格的 gate。不要为了中文化改写 canonical contract、命令、路径或 skill protocol。
