...
本 reference 在 review 已运行**之后**加载。在 ce-work Tier 2 path 中，step 2a 已经调用了 `ce-code-review`；此 apply step must consume the completed review，consume the completed review output，不要启动第二次 review，否则会浪费 reviewer dispatches，并有覆盖 Residual Work Gate 要 reconcile 的 artifact 的风险。
...
只有当 caller 到达本文件时**尚未**运行 review（手头没有 review output）时，才 invoke review as a cold caller，然后继续 apply。当 caller 已经运行过 review（例如 ce-work Tier 2 step 2a）时，不要调用。
