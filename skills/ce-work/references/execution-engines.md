...
> **中文导读：** 本文件是 execution-engine 选择与 tail ownership 的 canonical executable contract。请按英文原文执行；关键边界是先解析 routing、只在选中 cross-model engine 后加载协议，并始终由 host 保留 canonical integration、verification 与 shipping authority。
