# Tracker Detection and Defer Execution（Tracker 检测与延后执行）
...
## Probe Timing and Caching（Probe 时机与缓存）
...
## Label Logic（Interactive mode 标签逻辑）
...
Stem（问题主干）:
> Defer 失败：<tracker name> 返回 <error summary>。agent 应如何处理这个 finding？
...
| Tracker | Interface | Invocation sketch（调用草图） | Body format（正文格式） | Labels |
|---------|-----------|-------------------|-------------|--------|
