...
**中文说明：** `mode:pipeline` 不允许任何 blocking question。`needs-human` 的 decision context 写回对应 open thread，并作为 structured residual 返回 caller；禁止写入 PR-body residual section。Authority 只继承 caller 已获授权的 fix/commit/push/reply/resolve scope，不能扩大到 merge、rebase、force-push 或 approve CI。
