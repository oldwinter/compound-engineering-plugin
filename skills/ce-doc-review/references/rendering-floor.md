...
> **中文导读：** 这是 finding 在所有人类决策 surface 上的唯一事实源。固定顺序为 recommendation -> consequence -> evidence/options -> capped mechanism；需要打开原文才能理解的 ID、anchor 或 token，必须说明其 navigation/provenance/mechanism 功能，或移到 decision block 外。
