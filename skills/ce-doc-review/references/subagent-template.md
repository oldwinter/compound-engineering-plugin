...
`why_it_matters` field 是 reader 理解问题的入口：可能是 triage findings 的 developer、几个月后回到 doc 的 reader，或 downstream automated surface；他们不应为了理解问题而重新阅读整个 file。把它当作 output 中最重要的 prose field；每个 downstream surface（walk-through questions、bulk-action previews、Open Questions entries、non-interactive output）都依赖它足够清楚。
