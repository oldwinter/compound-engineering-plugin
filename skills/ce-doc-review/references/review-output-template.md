...
> **中文导读：** Batch report 以 severity 分组，并遵循 shared rendering floor。每行先给 reader 可行动的 recommendation 和 consequence，保留必要 evidence；mechanism 最后且限长。表格内 literal pipe 必须转义，避免破坏 reviewer/confidence/tier columns。
