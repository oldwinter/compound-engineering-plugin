...
> **中文导读：** Elevation 只把最重的 reasoning step 交给用户指定 model。上游本轮加强了 plan/brainstorm 输出中的 rule ownership：elevated author 必须避免跨 layer 重复 requirements，并用 Product Contract 的 `Governs R...` links 携带 session-settled decisions。
