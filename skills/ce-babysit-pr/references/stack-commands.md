...
**中文导读：** 本 reference 只服务于 fresh snapshot 已确认的 managed stack。它给出 read-only orientation、layer checkout、manager-owned sync，以及 `stack-land` 下 prefix merge 的精确命令；必须保持 downstack-to-upstack 顺序，draft 和 human blocker 不得被跨过，也不得把 `gh pr merge`、raw rebase 或 force-push 用在 managed members 上。下方英文是 canonical executable contract，命令与安全边界保持原样执行。
