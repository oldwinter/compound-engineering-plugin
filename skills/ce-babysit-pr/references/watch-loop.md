...
**中文导读：** 本 reference 定义每种 harness 如何 schedule tick、on-disk state contract、保证 idempotent/crash-safe 的 claim -> act -> confirm dedup protocol，以及 edge-case handling。每次 snapshot 还必须用 host-qualified exact Git Ref 独立验证 live base OID；不一致或探测失败会设置 `base_ref_blocker`，禁止 merge-ready 与 branch-currency mutation。Checkpoint、durable schedule 或跨 layer re-entry 必须保留同一个非 `target` posture，避免把 stack-aware run 降级成 target-only。Ordering invariant 由 `SKILL.md` 定义；这里的英文 mechanics 是 canonical executable contract，保持原样执行。
