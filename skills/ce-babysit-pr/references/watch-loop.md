...
**中文导读：** 本 reference 定义每种 harness 如何 schedule tick、on-disk state contract、保证 idempotent/crash-safe 的 claim -> act -> confirm dedup protocol，以及 edge-case handling。Ordering invariant 由 `SKILL.md` 定义；这里的英文 mechanics 是 canonical executable contract，保持原样执行。
