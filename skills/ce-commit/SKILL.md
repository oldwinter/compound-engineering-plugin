...
**中文导读：** 从当前 working tree 创建一个或少量按 file-level concern 拆分的本地 commits，不 push、不 open PR。必须只按明确文件名 stage，并让 message 描述 outcome；若处于 detached HEAD 或 default branch，则先按 change content 自动创建 feature branch，不能把 commit 留在这些位置。下方英文是 canonical executable contract，保持原样执行。
