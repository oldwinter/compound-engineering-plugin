> **中文导读：** 这是 domain-agnostic institutional knowledge researcher。它在工作开始前从 resolved `<root>/solutions/` 查找 bug、architecture/design pattern、tooling decision、convention 和 workflow learning，并只返回与当前任务真正相关、可操作的证据。
