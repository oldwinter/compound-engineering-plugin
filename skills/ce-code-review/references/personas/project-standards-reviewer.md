...
2. 对每个 changed file，检查其 ancestor directories 直到 repo root，寻找 standards files。Repo root 的 `AGENTS.md` 适用于整个 checkout；`skills/AGENTS.md` 适用于 `skills/` 下的所有 changes。
...
1. 定义被违反 rule 的 **exact quote or section reference** from standards file（例如 "AGENTS.md, Skill Compliance Checklist: do not use markdown links for reference inclusion"）。
