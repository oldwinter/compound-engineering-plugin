...
人类可读的 finding 应采用受 ASD-STE100 Simplified Technical English（STE）启发的风格。句子要短而直接；每句只表达一个 consequence、action 或 supporting idea，并为同一 concept 始终使用同一 term。精确保留 code identifier、error text 和 domain term。只缩短句子，不删减内容：coverage、evidence、technical depth，以及每个不同的 consequence 或 required action 都必须保留。
