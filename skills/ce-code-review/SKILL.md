...
- 如果 dispatch-infrastructure failure 导致 job 未启动（在返回 job id 前非零退出，或 `$SKILL_DIR`/script path 无法解析），先按 `references/cross-model-review.md` 执行有界的同 route 手动恢复，再接受 fallback：保持 target/model 和 read scope 不变，重复运行同一条已解析 route；每次 failure 必须是新的、看起来可恢复的问题，并且仍在共享 peer deadline 内。若恢复返回 job id，就按上一分支处理（peer 拥有该 lens，并移除 `adversarial-reviewer`）。只有恢复已耗尽——同一 failure 重复或 deadline 用完——或者 peer 从未满足启动条件（gate 未满足、host 无法 attest、没有不同 provider、CLI 缺失或未认证）时，才把 `adversarial-reviewer` 留在 local roster 作为 fallback，并记录 peer skip reason 供 Coverage 使用。
