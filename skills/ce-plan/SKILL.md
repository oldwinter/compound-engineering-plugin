...
一旦 plan 已识别且看起来 complete（所有 major sections present、implementation units defined）：
- **Routing 先基于 file extension，再看 frontmatter。** HTML plans（`.html`）始终是 software plans：html-rendering invariant 禁止 YAML frontmatter，因此 missing frontmatter 对 HTML 不是 non-software signal。将 visible-header metadata（title、date）视为 frontmatter equivalent。
...
本 phase 使用的所有 specialist research 和 deepening prompts 都是 `references/agents/` 下的 skill-local prompt assets。Dispatch 时，读取 matching file，并用该 prompt content 加上下方 task-specific context seed 一个 generic subagent。不要按 type/name dispatch standalone agents。

Model tiering 位于 caller 中，而不是 prompt assets 中。Local prompt files 没有 frontmatter。External/organizational research prompts（例如 `slack-researcher` 和 `web-researcher`）在当前 harness 暴露 known override 时使用平台 mid-tier model；否则省略 override 并继承。High-judgment architecture、migration 和 planning-deepening prompts 使用 inherited model，除非 harness 有 established cheaper capable tier。
...
并行运行这些 local prompt assets：

- `references/agents/repo-research-analyst.md` — scope: technology、architecture、patterns。传入 planning context summary。
- `references/agents/learnings-researcher.md` — 传入 planning context summary。

**Agent-native planning triage（conditional）** — 广泛考虑，选择性 dispatch。当 request、origin document 或 repo research 表明以下任一情况时，与 local research agents 并行 dispatch 一个 generic subagent，并用 `references/agents/agent-native-planning-strategist.md` seed：

- agent、assistant、chat、workflow automation、MCP、plugin、skill、tool registry、prompt 或 autonomous-loop work
- codebase 已有 agent surface，且该 feature 会改变 user-visible capabilities
- primary domain action 是 repetitive、high-volume、complex、naturally language-shaped，或很可能需要 automation access
- plan 有可能扩大 UI/API actions 与 agent-accessible tools/context 之间的 gap

不要为 cosmetic、layout-only、animation-only、brand、low-value preference，或在没有 agent surface 的 product 中的 narrow work dispatch。如果 signal borderline，不 dispatch；只在影响 high-value domain action 时携带一句 future parity consideration。将结果纳入 consolidation 作为 planning inputs，而不是 standalone advice appendix。
...
- 当 conditional triage dispatched 时的 agent-native planning findings：action/context parity decisions、tool/workspace/execution-lifecycle choices、scope boundaries 和 verification scenarios

**Slack context（opt-in，需用户明确启用）：** 绝不 auto-dispatch。按 condition route：

- **Tools available + user asked**：用 `references/agents/slack-researcher.md` 加 planning context summary dispatch 一个 generic subagent，并与其它 Phase 1.1 agents 并行。如果 origin document 有 Slack context section，verbatim 传入，让 researcher 聚焦 gaps。将 findings 纳入 consolidation。
...
**利用 `repo-research-analyst` local prompt 的 technology context：**

`repo-research-analyst` local prompt output 包含 structured Technology & Infrastructure summary。用它做更 sharper 的 external research decisions：

- 如果检测到 specific frameworks 和 versions（例如 Rails 7.2、Next.js 14、Go 1.22），将这些 exact identifiers 传给 `framework-docs-researcher` local prompt，让它 fetch version-specific documentation
...
如果 Step 1.2 表示 external research 有用，按 Stage 2 classified 的 **intent** dispatch。使用平台 subagent primitive（Claude Code 中的 `Agent`/`Task`、Codex 中的 `spawn_agent`）或可用等价物；否则 inline 或 serial 执行。读取 `references/agents/` 中选中的 prompt asset，并用它 seed 一个 generic subagent。对 `web-researcher.md`，传 focus hint 加 planning context summary，且 **不要** 传 codebase content：它在 external 范围工作。

- **Implementation-guidance**：并行运行：
  - `references/agents/best-practices-researcher.md`，传入 planning context summary。
  - `references/agents/framework-docs-researcher.md`，传入 planning context summary；如果 Phase 1.1 中有 exact frameworks/versions，也一并传入。
- **Landscape / option-discovery**：`references/agents/web-researcher.md`，传入 focus hint 和 planning context summary。当 request targets code host 上的 projects（例如 "competitors on GitHub"）时，在 focus hint 中命名 discovery dimensions：project names and URLs、release recency and activity、CLI/UX shape、install path、docs and examples、plugin/extension surfaces、recurring issue themes 和 license：star counts 仅作为 weak signal。
- **Mixed**：**sequential, not parallel**：先运行 `web-researcher` local prompt map landscape 并产出 shortlist；然后仅当 shortlisted technologies 的 details materially shape plan 时，针对它们运行 `framework-docs-researcher` 和/或 `best-practices-researcher` local prompts。

**Tool-unavailable handling（工具不可用处理）。** `web-researcher` 会 self-check web tools，缺失时 stop。绝不要因此 block：如果它报告 research unavailable，或任何 researcher fails，warn 并继续，并将 gap 带入 Phase 1.4，让 plan 诚实记录它；尤其是用户明确 requested external research 时，silent skip 会让 plan 看起来 evidence-based，但实际不是。
