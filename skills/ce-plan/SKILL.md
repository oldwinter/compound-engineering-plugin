...
**中文导读：** 把 feature brief 或 requirements-only unified plan enrich 成 implementation-ready plan：先 ground repository reality，再明确 architecture、files、tests、risks 和 implementation units，同时保留 session-settled decisions 与 Product Contract。Planning 负责 HOW，不重新发明已经在 brainstorm 中确定的 WHAT。下方英文内容是 canonical executable contract，必须按原文执行。
...
**Model elevation。** 编写 plan 前，加载并遵循 `references/reasoning-elevation.md`。它会判断 interpret-findings-then-author step 是否选定了 model——来源包括本次 run 的 prompt、被 strip 的 `plan_model:<alias>` caller carrier（例如由 LFG 传入，pipeline mode 也生效），或 `plan_model` config key——若已选定，就在任何 harness 上把这一步分派给所选 model，并进行 read-only verifying handoff，失败时透明回退到 session model。未选择 model 时它是 no-op，正常继续。在所有 harness 上行为相同，不要按 host 设置 gate。
