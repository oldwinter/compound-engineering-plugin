...
- [ ] 在 bash code blocks 中，用 `${CLAUDE_SKILL_DIR}` 构造 co-located script paths；不要用裸 relative path，也不要用 `${CLAUDE_PLUGIN_ROOT}`。Runtime Bash tool 从用户 project CWD 运行，不是 skill directory，所以裸 `bash scripts/my-script` 会 resolve 到 `<project>/scripts/` 并失败；这是 #764、#811、#898、#943 背后的 recurring bug class。
- [ ] `${CLAUDE_SKILL_DIR}` 只在 Claude Code 上 resolve；在其他 targets 上，包括会原样加载 `SKILL.md` 且忽略 `ce_platforms` 的 **native Codex plugin install**，它是 unset，因此那里找不到 bundled script。裸 `${CLAUDE_SKILL_DIR:-.}` shell default 会在 off-Claude 时静默运行不存在的 `./scripts/...` path，而不是可见地失败。对于 **optional guard script**，优先使用 existence guard `if [ -n "${CLAUDE_SKILL_DIR}" ] && [ -f "${CLAUDE_SKILL_DIR}/scripts/x" ]; then ... ; else <inline fallback>; fi`，让保护在 off-Claude 时仍能运行（例如 `ce-compound`）。如果某个 skill 的 core behavior 本身就是 bundled script，当 portability matters 时不要把 core behavior gate 在 runtime bundled-script call 上；详见 root `AGENTS.md` 的 *Platform-Specific Variables in Skills*、issue #943，以及下方 *Tool Selection in Agents and Skills* 中的 "Permission gate on extracted scripts"。
- [ ] 用 backtick *read-time* path 引用 script（例如 `` `scripts/my-script` ``），让 agents 能 locate 它并读取；read-time references 在所有 platforms 上都按 skill directory resolve。Bash code block 已提供 invocation，不需要 markdown link。
...
1. **Path resolution:** 永远不要相对 CWD 读取 config；用户可能从 subdirectory 调用 skill。始终用 `git rev-parse --show-toplevel` 从 repo root resolve。

2. **Worktrees:** Gitignored files 是 per-worktree 的。在 main checkout 中创建的 config file 不会存在于 worktrees。`--show-toplevel` 返回当前 worktree path，因此从 worktree 中找不到 main checkout 的 config。这可以接受：config 是 optional，使用 worktrees 的用户可以在那里添加 config file。

**Blessed pattern — 只 pre-resolve repo root，然后在 skill body 中读取文件：**
```
!`git rev-parse --show-toplevel 2>/dev/null || true`
```
在 skill body 中：如果 pre-resolved line 是 absolute path，把它用作 `<repo-root>`；如果为空，或仍显示 backtick command string，就在 runtime 通过 shell tool 运行 `git rev-parse --show-toplevel` resolve `<repo-root>`，然后用 native file-read tool（Claude Code 的 Read，Codex 的 read_file）读取 `<repo-root>/.compound-engineering/config.local.yaml`。Runtime fallback 是 load-bearing：`!` pre-resolution 仅限 Claude Code，且**不会**被 converters 翻译（`transformContentForCodex` 和其他 target writers 会把 `!`cmd`` 作为 literal text 复制），所以在 Codex/Gemini/Pi/OpenCode 上该行 unresolved，agent 必须自己 derive root；否则 config 会在所有非 Claude platforms 上被静默忽略。只有无法 resolve root（不在 git repo 中）或 file 缺失时，才是 "no config" -> fall through to defaults；绝不要因为 missing config fail 或 block。（这与 `ce-sessions` pre-resolution fallback 一致。）

**不要在 `!` pre-resolution 本身读取 config contents。** 两个早期形式现在会被拒绝，并由 `tests/skill-shell-safety.test.ts` 强制：

- `` !`cat "$(git rev-parse --show-toplevel 2>/dev/null)/.compound-engineering/config.local.yaml" 2>/dev/null || echo '__NO_CONFIG__'` `` 会在 permission checker 拒绝 `$()` command substitution 的环境中 abort skill load（Windows / CC 2.1.63 上 issue #920 已报告）。
- `(top=$(...); cat "$top/...")` subshell variant 更糟：即使在 subshell 内，`;` 也会被拒绝为 `Unhandled node type: ;`（issue #758）。

Blessed pattern 会避开两者：裸 `git rev-parse` first token 没有 `$()`、没有 `;`，并匹配 common allow rules；config contents 则在 runtime 用 native tool 读取（不经过 skill-load checker）。之前通过 `git-common-dir` 和 `${common%/.git}` derive main repo root 的 worktree fallback 也不可用，因为 parameter expansion operators 会被拒绝为 "Contains expansion"（见上方 Pre-resolution exception）。
...
当在 `compound-engineering` 旁添加新 plugin 时，repo 会 ship 到三种 marketplace formats（Claude、Cursor、Codex）。三者必须保持 parity，否则下次运行 `bun run release:validate` 会失败。Checklist：
