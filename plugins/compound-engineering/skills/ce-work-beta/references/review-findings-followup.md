...
当 Tier 2 `ce-code-review` 已完成，且 **ce-work-beta** 应在 Residual Work Gate 前 apply fixes 时，加载本 reference。
...
本 reference 在 review 已运行**之后**加载。在 ce-work-beta Tier 2 path 中，step 2a 已经调用了 `ce-code-review`；此 apply step must consume the completed review，consume the completed review output，不要启动第二次 review，否则会浪费 reviewer dispatches，并有覆盖 Residual Work Gate 要 reconcile 的 artifact 的风险。
...
只有当 caller 到达本文件时**尚未**运行 review（手头没有 review output）时，才 invoke review as a cold caller，然后继续 apply。当 caller 已经运行过 review（例如 ce-work-beta Tier 2 step 2a）时，不要调用。
...
Orchestrator **does not investigate findings**（不预读 cited files 来判断 complexity 或 inline vs subagent）。那会消耗本来要保护的 context window。
...
2. **Group by `file`。** 同一 file 上的所有 eligible findings -> **一个 subagent**（它只加载一次文件，并按 severity order 处理其 `#` list）。
3. **Parallel waves：** 具有 **disjoint file sets** 的 batches 可以 parallel run（与 `ce-work-beta` SKILL.md Phase 1 Step 4 的 same worktree / shared-directory rules 相同）。
