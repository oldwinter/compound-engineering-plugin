...
- **Extraction tier** — grounding scout：retrieval 和 quoting work。使用平台 cheapest capable model（Claude Code 中为 `model: "haiku"`；Codex 中为最快的 mini-class model；Gemini 中为 flash-class）。"Capable" 是 spec 的一部分：当 repo 很大或 stack obscure 时，升级到 generation tier。
