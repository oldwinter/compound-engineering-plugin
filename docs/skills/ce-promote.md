...
它只起草，不发布。它永远不会 post、publish、commit 或 open PR；真正 ship copy 是 human action。
...
- 只需要 internal release notes；plugin release history 使用 `/ce-release-notes`
...
- [`ce-release-notes`](./ce-release-notes.md) - plugin 的 internal release history（不同 audience：developers，而不是 end users）
- [`ce-demo-reel`](./ce-demo-reel.md) - 为已 ship feature 捕获 visual evidence 供 PR 使用（很适合搭配 announcement copy 的 visual）
