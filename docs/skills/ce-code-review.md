...
**由哪个 provider 运行 peer** 会自动选择，也允许 override。它与 `ce-doc-review` 使用同一 independence system，但只覆盖 adversarial lens（没有 security/correctness twins，也没有 whole-diff peer）。Skill 只 attest host provider 以排除它；无法 attest 的 host 会 skip，避免误用 same-provider peer。优先级是：conversation → `.compound-engineering/config.local.yaml` 中的 `cross_model_peer:` → context 中的 project instructions → 首个可用的 `codex → claude → grok → composer`。只有显式设置 `CROSS_MODEL_MAX_PEERS=2` 才会启用第二个 provider。Peer 以 in-tree read-only 方式对照 diff base review 当前 work tree；reviewed code 可能发送给所选 provider。Peer 是 detached、supervised job，review 以 bounded slices 轮询，因此不会有单次 tool call 覆盖其整个 runtime；已经启动但失败或 timeout 的 peer 会在 Coverage 中显示 terminal state，不会静默消失。Report 会记录 requested-vs-served model identity；没有 served-model receipt 的 route 会标记为 `requested, unverified`，而不是把 requested model 当成事实。该 pass 是 non-blocking 的：CLI 缺失、timeout 或 output 无法 parse 都不会让 review 失败。
...
当发现的 plan 带有 `session-settled:` KTD 时，synthesis 会把仅仅偏好另一种 approach 的 finding 放进 report-only queue，并打上指明对应 KTD 的 `settled_conflict` stamp。所有 mode 都这样处理，包括 interactive apply，因此用户已经作出的 decision 不会因为 reviewer 的个人偏好而在 gate 中被丢弃。Settled approach 内的真实 defect 仍保留完整 severity；如果 evidence 表明某个 settled decision 根本不可行，则会被显著展示，以便上游 pipeline gate 据此停止。Reviewers 本身看不到这些 annotations：reviewer bundles、intent summary 以及 cross-model adversarial pass 都会排除它们；orchestrator 在 review 后统一 triage，避免任何 lens 因为知道某个选择已获认可而产生锚定偏差。
