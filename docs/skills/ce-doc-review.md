...
`ce-code-review` review diffs（code changes）；`ce-doc-review` review docs（requirements、plans）。不同 reviewer personas、不同 findings shape、不同 routing。二者共享 multi-persona dispatch + synthesis pattern，但针对各自 artifact types 调校。
