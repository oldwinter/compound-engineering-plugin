...
**由哪个 target 运行 peer** 会自动选择，也允许 override。Host harness 与 serving family 分开跟踪，以排除 same-model checks。优先级为 conversation、`.compound-engineering/config.local.yaml` 中的 `cross_model_peer:`、active project instructions，再到 `codex -> claude -> grok -> composer`。也可显式使用 `Cursor`，表示由 `cursor-agent` 使用 configured default/Auto model；`Composer` 表示经 Cursor 使用 Composer model；Grok 优先 native CLI，也可走 sanctioned Grok-via-Cursor route。除非 receipt 证明 serving family 不同，否则 Cursor Auto 不会提升为 independent corroboration。共享 local-config contract 参见[配置参考](./configuration.md)。
