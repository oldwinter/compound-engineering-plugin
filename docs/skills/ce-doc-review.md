...
**由哪个 target 运行 peer**会自动选择，也允许 override。Host harness 与 serving family 分开跟踪，使该 pass 可以排除 same-model checks。优先级是 conversation、`.compound-engineering/config.local.yaml` 中的 `cross_model_peer:`、active project instructions，然后是 `codex -> claude -> grok -> composer`。也可以显式使用 `Cursor`，表示由 `cursor-agent` 使用 configured default/Auto model；`Composer` 表示经 Cursor 使用 Composer model；Grok 优先使用 native CLI，也可以走 sanctioned Grok-via-Cursor route。除非 receipt 证明 Cursor Auto 的 serving family 与 host 不同，否则不能把它提升为 independent corroboration。
