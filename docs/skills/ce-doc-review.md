...
**由哪个 provider 运行 peer**会自动选择，也允许 override。Skill 仅 attest *host 自身*的 provider 以排除它，确保该 pass 不会 self-review；若无法 attest host（例如 Cursor 使用无法识别的 model），则跳过，避免误用 same-provider peer。随后，它会按优先级为每个 document 只 resolve **一个**不同 provider：conversation 中声明的 preference；`.compound-engineering/config.local.yaml` 中的 `cross_model_peer:` key；project active instructions 中已有的 preference；最后按 `codex → claude → grok → composer` 顺序选取首个可用 provider。Provider 通过 dedicated CLI（`codex`、`claude`、`grok`）或 `cursor-agent`（grok fallback 与 Composer）访问。每个 activated lens 在每个 provider 上都使用一个 high-reasoning model（`gpt-5.6-sol`、`opus`、`grok-4.5` 或 `composer-2.5-fast`；Composer 的 `-fast` tier 就是其 ceiling），具体 ID 集中在 script 内的一份 mapping 中。只有显式设置 `CROSS_MODEL_MAX_PEERS=2` 才会启用第二个 provider。每个 peer 都作为 detached、supervised job 运行，review 通过 bounded wait slices 轮询；没有任何单次 tool call 覆盖 peer 的整个 runtime，已启动后失败或 timeout 的 peer 会连同 terminal state 一起出现在 Coverage 中，而不会静默消失。该 pass **non-blocking**：无法 attest host、没有可用的不同 provider、CLI 缺失或未认证、以及 timeout，都会静默跳过，review 仍按 single-model 路径完成。Interactive 和 headless modes 都会运行它：interactive 会宣布这是一次 **independent cross-model review**，列出 requested model；如果经 `cursor-agent` route，还会明确区分 Grok-via-cursor-agent 与 Composer，并且**说明完整 document content 会发送给该 provider**。Headless 不打扰用户，但仍为这次 egress 写 audit log。Report 会记录 requested-vs-served model identity；没有 served-model receipt 的 route 标记为 `requested, unverified`，而不会把 requested model 当成已验证事实。

**Trust boundary：**该 pass 会把完整 document content 嵌入 peer prompt，并发送给外部 model provider（取决于最终 peer，可能是 OpenAI、Anthropic、xAI 或 Cursor）；`CROSS_MODEL_PEERS` 限制哪些 providers 可以接收内容（未设置时采用默认顺序，设置后作为 allowlist）。Peer 从没有 project context 的空 scratch dir 启动，并保持严格 read-only；所有 routes 都拒绝 writes、network、MCP 和 subagents。Read 权限分为两级：**truly tool-less** 的 claude（`--bare --tools ""`，禁用所有 built-ins，不自动发现 CLAUDE.md/MCP）与 grok（拒绝 `Read`/`Edit`/`Write`/`Bash`/`Task`/web/`mcp__*`），完全没有 read tool；以及仍有少量 read 能力的 **read-only residual** routes：codex（`-s read-only`）与 cursor-agent（`--mode ask --sandbox enabled`），它们仍允许 read tool，codex 还允许 read-only shell exec。因此，影响边界是信息披露，而不是 repo mutation；script 会为每次 cross-model send 输出一行 audit log，headless mode 下也可审计 egress。Peer prompt 仅使用 document basename，因为内容已嵌入 prompt；超大 document 会干净地 skip，而不是截断。对于本项目的 own-document threat model，codex/cursor-agent routes 的 read residual 是**可接受的**：被 review 的 doc 属于 maintainer，host agent 本来就以比 peer 更高的权限在 repo 内运行，因此 peer 能读取文件不会增加实质暴露面。
