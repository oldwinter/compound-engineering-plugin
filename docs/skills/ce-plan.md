...
# Session 继续使用 Opus 等 model，只把 heavy planning step 交给 Fable，
# 在收益最大的地方精准使用强但昂贵的 model
/ce-plan 把通知静音 requirements 转为 implementation-ready plan，使用 fable
```
...
| `<requirements-only plan path>` | 原地 enrich 同一 unified plan |
| `<legacy requirements doc path>` | Origin-sourced planning，写入新的 unified plan |
| `<plan path>` | Resume offer（intent 匹配时 deepen） |
...
| `output:html` | 将 plan 写成单个 self-contained HTML file，而不是 markdown。互斥：plan 是 `.md` 或 `.html`，绝不同时生成。Default 是 markdown。设置 `.compound-engineering/config.local.yaml` 的 `plan_output: html` 可令 HTML 成为 default。Pipeline mode（LFG、`disable-model-invocation`）始终强制 markdown，使 downstream automation 获得 stable text shape。参见[配置参考](./configuration.md)。 |
| `confirm:auto` | 跳过本次 run 的 pre-plan scoping-confirmation pause。ce-plan 会为自己写 scope summary，将 inferred scope 记录到 `Assumptions` section，宣布继续并不等待。只跳过该 confirmation；genuine blockers 和 post-plan menu 仍会出现。用 `confirm:ask` 可为单次 run 强制开启 gate；在 `.compound-engineering/config.local.yaml` 设置 `plan_skip_scoping_confirm: true` 可令跳过成为 default。 |
