...
```text
# 从自然语言 outcome 开始，共同构建 measurement spec
/ce-optimize reduce build time by 30%

# 使用 LLM-as-judge rubric 优化 qualitative result
/ce-optimize improve clustering quality for notification categories

# 从现有、可 review 的 optimization specification 运行
/ce-optimize path/to/clustering-quality.yaml

# 从已保存 specification 恢复 persisted run
/ce-optimize .context/compound-engineering/ce-optimize/clustering-quality/spec.yaml
```

如果 metric、gates、budget 或 stopping rule 必须在 experiments 开始前经过 review，请使用 spec。
...
- **Resume an existing run**：`/ce-optimize .context/compound-engineering/ce-optimize/<spec-name>/spec.yaml` 检测 existing branch，并提供 Resume vs Fresh Start
