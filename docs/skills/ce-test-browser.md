...
`ce-code-review` 只有 `mode:agent` 是与此 skill 在同一 checkout 并发运行安全的 review mode；interactive review 可能 mutate checkout，干扰 running dev server state。
...
只有 code review 使用 read-only 的 `mode:agent` 时可以。Interactive review 可能 mutate checkout，破坏 running dev server state。要么配合 read-only review，要么在 isolated worktree 中分开运行。

---

## 另见

- [`ce-code-review`](./ce-code-review.md) - 可为 browser-affecting PRs spawn 此 skill（同一 checkout 并发时使用 `mode:agent`）
