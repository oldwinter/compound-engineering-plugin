...
> **中文导读：** `ce-explain` 的交付物是一份可长期保留、self-contained 的 visual document，而不是聊天里的临时说明。它必须明确写给某类读者，用证据 grounding，但不得把 thread IDs、commit SHAs 等 opaque tokens 塞进正文。Check-in 是 opt-in：diff 用 predict-then-reveal，其它材料用会被纠正的 exercises；多数 run 只产出文档。目标位置按当前 session capability 选择，local file 始终可用。
