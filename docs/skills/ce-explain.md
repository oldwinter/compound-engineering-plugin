...
Explainer 以 HTML-first（按需 markdown）生成，默认 show-n-tell：结构用 diagram，代码用 annotated snippet，recap 用 timeline。Skill 询问目标位置前，会先写入稳定 temp location。Claude Code 在相应 tool 存在时优先使用 private Claude Artifact；其他 harness 优先使用经明确确认的 public ht-ml.app page。Local file、Proof、Thinkroom 和 leave-in-temp route 仍是 capability-gated fallbacks。

## 新颖之处

1. **Predict-then-reveal for diffs。** Turn 在展示 raw change 并要求你预测后 *结束*。任何 interpretive content 都不会提前泄露；随后 reveal 会精确指出 prediction 漏掉的内容。命名这个 gap 本身就是教学。
2. **Check-in 位于 session，而不是 artifact。** Doc 保持 display-only；exercise 在 chat 中提出，skill 可以检查并纠正答案。“Just the explainer”是推荐的第一选择，quiz 是 opt-in 第二选择。
3. **设计为可跳过。** Routine recap 跳过 check-in；你始终可以拒绝。有些内容不需要 learning loop。
4. **Capability-detected destinations。** Claude Code 优先 Claude Artifact；其他地方在 explicit public-content warning 后由 ht-ml.app 提供 harness-independent public URL。Local file 是始终存在的底线，且 artifact 会在询问前落盘，所以全部拒绝也不会丢失。
5. **诚实的 external grounding。** 无 web access 的 external topic 会 fallback 到 model knowledge，并在 artifact 中标为 unverified，绝不伪装成已检查。
