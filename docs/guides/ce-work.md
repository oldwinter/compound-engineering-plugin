...
> **中文更新摘要：** `prefer` 与 `require` 都会按顺序尝试 external implementation route；route 不可用时，披露一次后回退到当前 harness/session model。只要 route 仍可用，`require` 就固定用户请求的 external identity，并且绝不会改用未请求的 external recipient。
